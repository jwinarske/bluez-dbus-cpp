...
namespace org::bluez {
    ...
    class LEAdvertisement1 :
        ...
        bool registered_{};
        static constexpr auto INTERFACE_NAME = "org.bluez.LEAdvertisement1";
    ...
    public:
        ...
        static LEAdvertisement1 &create(IConnection &connection, std::string objectPath) {
            const auto self = new LEAdvertisement1{connection, std::move(objectPath)};
            return *self;
        }
        ...
        std::shared_ptr<LEAdvertisement1> registerWith(std::shared_ptr<LEAdvertisingManager1> manager,
                                                       const std::function<void(const Error *)> &
                                                               onRegistrationCallback = nullptr) {
         ...
         */
        LEAdvertisement1 &withServiceData(std::map<std::string, Variant> data) {
         ...
         */
        LEAdvertisement1 &withManufacturerData(std::map<uint16_t, Variant> manufacturerData) {
         ...
         *              0x26                   0x01         0x01...
         */
        LEAdvertisement1 &withData(std::map<uint8_t, Variant> data) {
            data_ = std::move(data);
         ...
         */
        LEAdvertisement1 &withDiscoverable(const bool discoverable) {
         ...
         */
        LEAdvertisement1 &withAppearance(const uint16_t appearance) {
         ...
         */
        LEAdvertisement1 &withDuration(const uint16_t duration) {
            duration_ = duration;
         ...
         */
        LEAdvertisement1 &withTimeout(const uint16_t timeout) {
            timeout_ = timeout;
        ...
        LEAdvertisement1 &withSecondaryChannel(std::string secondaryChannel) {
            secondaryChannel_ = std::move(secondaryChannel);
    ...
    protected:
        ...
        std::map<std::string, Variant> ServiceData() { return serviceData_; }
        std::map<uint16_t, Variant> ManufacturerData() { return manufacturerData_; }
        std::map<uint8_t, Variant> Data() { return data_; }
        std::vector<std::string> SolicitUUIDs() { return solicitUUIDs_; }
        bool Discoverable() const { return discoverable_; }
        uint16_t DiscoverableTimeout() const { return discoverableTimeout_; }
        std::vector<std::string> Includes() { return includes_; }
        std::string LocalName() { return localName_; }
        uint16_t Appearance() const { return appearance_; }
        ...
        std::map<std::string, Variant> serviceData_;
        std::map<uint16_t, Variant> manufacturerData_;
        std::map<uint8_t, Variant> data_;
