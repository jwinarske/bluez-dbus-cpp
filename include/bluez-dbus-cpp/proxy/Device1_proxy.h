...
namespace org::bluez {
    ...
    protected:
        explicit Device1_proxy(IProxy &proxy)
            : proxy_(proxy) {
        }
    ...
    public:
        ...
        [[nodiscard]] ObjectPath Adapter() const {
            return proxy_.getProperty("Adapter").onInterface(INTERFACE_NAME);
        }

        [[nodiscard]] bool LegacyPairing() const {
        ...
        std::map<uint16_t, Variant> ManufacturerData() {
            return proxy_.getProperty("ManufacturerData").onInterface(INTERFACE_NAME);
        }

        std::map<std::string, Variant> ServiceData() {
        ...
        std::map<uint8_t, Variant> AdvertisingData() {
