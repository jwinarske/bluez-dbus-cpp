...
namespace org::bluez {
    ...
    GattService1::GattService1(const std::shared_ptr<GattApplication1> &app, const std::string &serviceName,
        ...
        for (const auto &chrc: characteristics_) {
            if (chrc == characteristic)
                throw std::invalid_argument(
                    std::string("GattService1::addCharacteristic '") + characteristic->getPath() + std::string(
                        "' already registered!"));
        }
    ...
    void GattService1::addSubService(std::shared_ptr<GattService1> service) {
        ...
        for (const auto &path: includes_) {
            if (path == srvPath)
                throw std::invalid_argument(
                    std::string("GattService1::addSubService '") + srvPath + std::string("' already registered!"));
        }
